...
//Connection Settings @0-FA60F0DB
$CCConnectionSettings = array (
    "localhost" => array(
        "Type" => "MySQL",
        "DBLib" => "MySQL",
        "Database" => "myphytos_pes",
        "Host" => "localhost",
        "Port" => "3306;Database=myphytos_pes",
        "User" => "myphytos_pes",
    ...
    "siteground" => array(
        "Type" => "MySQL",
        "DBLib" => "MySQL",
        "Database" => "myphtos_pes",
        "Host" => "localhost",
        "Port" => "3306;Database=myphtos_pes",
        "User" => "myphytos_pes",
        "Password" => "Password123!!!",
        "Persistent" => false,
        "DateFormat" => array("yyyy", "-", "mm", "-", "dd", " ", "HH", ":", "nn", ":", "ss"),
...
//Initialize Common Variables @0-A2E58928
...
$CCSLocales->AddLocale("en-GB", Array("en", "GB", array("Yes", "No", ""), 2, ".", ",", array("January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"), array("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"), array("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"), array("Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"), array("S", "M", "T", "W", "T", "F", "S"), array("dd", "/", "mm", "/", "yyyy"), array("dd", " ", "mmmm", " ", "yyyy"), array("HH", ":", "nn"), array("HH", ":", "nn", ":", "ss"), "AM", "PM", 1, false, "", "windows-1252", "CP1252", array(1, 7)));
$CCSLocales->DefaultLocale = strtolower("en-GB");
$CCSLocales->Init();
$Charset = "";
...
$CCSDesign = "";
define("CCS_ENCRYPTION_KEY_FOR_COOKIE", 'Password123!!!');
...
//localhost Connection Class @-595562BA
class clsDBlocalhost extends DB_Adapter
{
    function clsDBlocalhost()
    {
        $this->Initialize();
    }
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["localhost"]);
        parent::Initialize();
...
//End localhost Connection Class

//siteground Connection Class @-34C1102C
class clsDBsiteground extends DB_Adapter
{
    function clsDBsiteground()
    {
        $this->Initialize();
    }
    ...
    {
        global $CCConnectionSettings;
        $this->SetProvider($CCConnectionSettings["siteground"]);
        parent::Initialize();
        $this->DateLeftDelimiter = "'";
        $this->DateRightDelimiter = "'";
    }
...
//End siteground Connection Class
...
function CCSecurityRedirect($GroupsAccess, $URL)
{
    ...
    {
        if(!strlen($URL))
            $Link = ServerURL . "secure/login.php";
        else
            $Link = $URL;
...
function CCSetALCookie($login, $password) {
    ...
    CCSetCookie("pestechlogin", $result, time() + CCS_EXPIRATION_DATE, "/", "", false);
}
//End CCSetALCookie

//CCRefreshALCookie @0-36A2CCF7
function CCRefreshALCookie($expirationDate) {
    if (CCS_SLIDING_EXPIRATION) {
        if (($expirationDate - (CCS_EXPIRATION_DATE / 2)) > time()) {
            list($login, $password, $expDate) = CCParseALCookie("pestechlogin");
...
//CCCheckAutoLoginCookies @0-59ED112D
function CCCheckAutoLoginCookies() {
    if (!CCGetUserLogin()) {
        $parts = CCParseALCookie("pestechlogin");
...
//CCLoginUser @0-B1F81AA3
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBlocalhost();
    $SQL = "SELECT Id, level, password FROM users WHERE username=" . $db->ToSQL($login, ccsText) . " AND password=MD5(" . $db->ToSQL($password, ccsText) . ")";
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("UserID", $db->f("Id"));
        CCSetSession("UserLogin", $login);
        CCSetSession("GroupID", $db->f("level"));
